      ...
      <div className="App">
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={divStyle}>
          ...
          <Controller
            ...
            render={({ field }) =>
              <TextField
                label="Where"
                type={"text"}
                error={errors?.where?.type === 'required'}
                variant="outlined"
                {...field} />
            }
          />
          ...
          <Controller
            ...
            render={({ field }) => (
              <FormControl>
                <InputLabel id="local-team">Local Team</InputLabel>
                <Select
                  {...field}
                  error={errors?.visitorTeam?.type === 'required'}
                  labelId="local-team"
                  label="Local Team"
                  multiple
                  renderValue={(selected) => (
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                      {selected.map((id) => (
                        <Chip key={id} label={players?.find(e => e.id === id).name} />
                      ))}
                    </Box>
                  )}
                  defaultValue={[]}>
          ...
          <Controller
            ...
            render={({ field }) => (
              <FormControl>
                <InputLabel id="visitor-team">Visitor Team</InputLabel>
                <Select
                  {...field}
                  error={errors?.visitorTeam?.type === 'required'}
                  labelId="visitor-team"
                  label="Visitor Team"
                  multiple
                  renderValue={(selected) => (
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
